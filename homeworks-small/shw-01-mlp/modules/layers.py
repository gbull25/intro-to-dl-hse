...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        batch_size = input.shape[0]
        dx_norm = grad_output * (self.weight if self.affine else np.ones(grad_output.shape[1]))
        if self.training:
            return (1. / batch_size) * self.inv_sqrt_var * (
                batch_size * dx_norm - 
                np.sum(dx_norm, axis=0) - 
                self.norm_input * np.sum((dx_norm * self.norm_input), axis=0)
            )
        return dx_norm / np.sqrt(self.running_var + self.eps)
